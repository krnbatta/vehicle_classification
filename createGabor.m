function[gaborSquareEnergy, gaborMeanAmplitude] = createGabor(im,nscale,norient)

    % Inititalise default values    
...
count=1;
gaborSquareEnergy=[norient*nscale];
gaborMeanAmplitude=[norient*nscale];
    % The main loop...
    for o = 1:norient                    % For each orientation.
        for s = 1:nscale                 % For each scale.
            
            filter = logGabor{s} .* spread{o};         % Multiply radial and angular components to get filter.
            EO{s,o} = ifft2(imagefft .* filter);       %Convolve image with even and odd filters returning the result in EO
